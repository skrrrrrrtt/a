...
# ==========================================
# JOIN WITH RETRY (Private Server Support)
# ==========================================
...
join_game() {
    ...
    # Build the deep link based on mode
    local deep_link
    if [ "$JOIN_PRIVATE_SERVER" = true ]; then
        # Get the private server code for this instance (rotating through available codes)
        local ps_index=$((instance_index % ${#PRIVATE_SERVER_CODES[@]}))
        local ps_code="${PRIVATE_SERVER_CODES[$ps_index]}"
        deep_link="roblox://experiences/start?placeId=$PLACE_ID&linkCode=$ps_code"
        log_info "  Using private server: ${ps_code:0:12}..."
    else
        # Public server
        deep_link="roblox://experiences/start?placeId=$PLACE_ID"
